...
const getUiDemo = (config, argv) => {
    ...
    if(!config.uiPath) {
        handleError('can\'t find uiPath in webuild.config.js');
        return;
    }

    if(!file) {
        handleError('use the command like this: ' + 'webuild ui [path]');
        return;
    }

    if(!test('-f', webpackConfigPath)) {
        handleError('can\'t find webpack configuration for ui');
        return;
    }
